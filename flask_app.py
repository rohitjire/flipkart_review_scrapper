    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if reviews.count() > 0:  # if there is a collection with searched keyword and it has records in it
                ...
                # commentboxes = prod_html.find('div', {
                #     'class': "col _39LH-M"})  # finding the HTML section containing the customer comments
                # reviews_link = "https://www.flipkart.com" + commentboxes.a[
                #     'href']
                # reviewRes = requests.get(reviews_link)
                # review_html = bs(reviewRes.text, "html.parser")
                # reviewBoxes = review_html.findAll("div", {"class": "_3gijNv col-12-12"})
                # only_reviewsBoxes = reviewBoxes[1:]
